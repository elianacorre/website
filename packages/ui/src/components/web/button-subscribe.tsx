...
import { Children, forwardRef, isValidElement, type MouseEvent, type PropsWithChildren, useState } from "react";
...
export const SubscribeButton = forwardRef<HTMLButtonElement, SubscribeButtonProps>(
	({ subscribeStatus = false, onClick, className, children, ...props }, ref) => {
		...
		if (Children.count(children) !== 2 || !Children.toArray(children).every((child) => isValidElement(child) && child.type === "span")) {
