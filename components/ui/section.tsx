...
import { SectionTitleEffect, SectionTitleEffectClassName, SectionTitleEffectProps } from "./section.title-effect";

// STYLES **********************************************************************************************************************************
export const SECTION_CONTENT = tv({
  base: `flex flex-col gap-8 font-light text-balance text-center
  sm:text-lg
  lg:text-start
  2xl:text-xl`,
});
...
export function Section({ children, className: C = {}, intent = "default" }: SectionProps) {
...
export const SECTION_IMAGE = tv({
  slots: {
    BASE: `relative hidden aspect-square max-w-full rounded-2xl border bg-white p-3 shadow-2xl 
    md:p-4 
    lg:flex lg:max-w-md lg:transition-transform
    xl:max-w-xl
    2xl:max-w-2xl`,
...
export function SectionImage({ className: C = {}, image, reverse = false }: SectionImageProps) {
  const { BASE, IMG } = SECTION_IMAGE();

  return (
    <figure className={BASE({ reverse, className: C.BASE })}>
      <Img {...image} className={IMG({ className: C.IMG })} />
    </figure>
  );
}

type SectionImageProps = Omit<ComponentProps<"figure">, "className"> & SectionImageStyles & { image: Image };
...
export const SECTION_TITLE = tv({
  base: `flex flex-col items-center text-4xl font-extrabold
  sm:text-6xl
  lg:items-start
  2xl:text-7xl`,
});
